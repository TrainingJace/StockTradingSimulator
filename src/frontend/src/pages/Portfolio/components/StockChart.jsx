...
function StockChart({ symbol, data, loading }) {
  ...
  // 过滤买入和卖出数据并打印调试信息
  const buyData = data.filter(d => d.action === 'buy');
  const sellData = data.filter(d => d.action === 'sell');
  
  console.log(`=== Chart Debug for ${symbol} ===`);
  console.log('All data points:', data.length);
  console.log('Buy data points:', buyData.length, buyData);
  console.log('Sell data points:', sellData.length, sellData);
  console.log('===========================');
